...
contract Factory {
  ...
  */
  function payToken(address _party, uint _amount, bool _long) public {
    require(created_contracts[msg.sender] == true);
    ...
    drct_interface.pay(_party, _amount);
  }
  ...
  */
  ...
  function calculatePayout() internal {
    ...
    uint num_DRCT_longtokens = DRCT_Token_Interface(long_drct).total_supply();
    uint num_DRCT_shorttokens = DRCT_Token_Interface(short_drct).total_supply();
  ...
  */
  ...
  uint swapamount;
  function changeContract(uint _num,bool _short){
    address swapcontract = contracts[_num];
    swap = TokenToTokenSwap_Interface(swapcontract);
    if swapcontract.swap_state() != tokenized{
      swapcontract.createTokens();
    }
    if (_short){
      swapamount.mul(995).div(1000) = Wrapped_Ether1.balanceOf(swapcontract);
      } 
    else{
      swapamount.mul(995.div(1000) = Wrapped_Ether1.balanceOf(swapcontract);
    }
  ...
  function forcePay(uint _begin, uint _end) public returns (bool) {
    //Calls the Calculate function first to calculate short and short shares
    if (share_short + share_short == 0)
    {
      Calculate();
    }
    //The state at this point should always be SwapState.ready
    require(msg.sender == owner);
    //Loop through the owners of short and short DRCT tokens and pay them
    ...
    for(uint i = _begin; i < loop_count; i++) {
      address short_owner = token.getHolderByIndex(i);
      uint to_pay_short = token.getBalanceByIndex(i);
      uint k;
      while(k<1) {
            uint to_pay_short = token.getBalanceByIndex(i);
            if (to_pay_short > swapamount){
            to_pay_short = swapamount;
            }
            else{
              k = 1;
            }
            assert(i == token.getIndexByAddress(short_owner));
            if (to_pay_short > 0){
              if (pay_to_short_a>0){
                swap.paySwap(short_owner, to_pay_short.mul(pay_to_short_a), true);
              }
              if (to_pay_short_b>0){
                swap.paySwap(short_owner, to_pay_short.mul(pay_to_short_b), false);
              }
              payToken(short_owner,to_pay_short,true);
            }
            if(k==0){
              swapamount -= to_pay_short;
              while (swapamount=0){
                contract_num += 1;
                changeContract(contract_num,true);
              }
            }
      }
    }
    ...
    //How do I get paid? Loop through last swaps once everyone else is paid out

    if (loop_count == count){
      contract_num = 0;
      for (uint x = 0; x < num_contracts; x++)
        changeContract(0);
        swapContract.endSwap();
      }
    return true;
  }
...
contract TokenToTokenSwap {
  ...
  */
  ...
  function paySwap(address _receiver, uint _amount, bool _is_long) public{
    require(msg.sender == factory_address);
    if (_is_long) {
        token_a.transfer(_receiver, _amount);
    } 
    else {
        token_b.transfer(_receiver, _amount);
    }
  }


      function endSwap() public {
