...
interface DRCT_Token_Interface {
  ...
  function getHolderByIndex(uint _ind, address _swap) external constant returns (address);
  function getBalanceByIndex(uint _ind, address _swap) external constant returns (uint);
