...
contract('Contracts', function(accounts) {
  ...
  beforeEach('Setup contract for testing', async function () {
    ...
    await DRCT_Token.new(factory.address);
    await DRCT_Token2.new(factory.address);
  ...
  it("Down Move", async function(){
  	await oracle.StoreDocument(1543881600,1000);
    await oracle.StoreDocument(1544486400,500);
    var balance1 = await (web3.fromWei(web3.eth.getBalance(account_two), 'ether').toFixed(0));
  	var balance2 = await (web3.fromWei(web3.eth.getBalance(account_three), 'ether').toFixed(0));
    assert.equal(await oracle.RetrieveData(1543881600),1000,"Result should equal end value");
    assert.equal(await oracle.RetrieveData(1544486400),500,"Result should equal start value");
	console.log("Contracts deployed successfully")
	  	var receipt = await factory.deployContract({from: account_two, gas:4000000});
	  	swap_add = receipt.logs[0].args._created;
	  	await userContract.Initiate(swap_add,10000000000000000000,10000000000000000000,0,true,{value: web3.toWei(10,'ether'), from: account_two});
	...
	console.log("Contracts deployed successfully");
	  	var receipt = await factory.deployContract({from: account_four, gas:4000000});
	  	swap_add = receipt.logs[0].args._created;
	...
	it("Test Multiple Swaps", async function(){
		...
		console.log('Entering Second and Third Swap')
		  	...
		  	assert.equal(await swap2.current_state.call(),3,"Current State should be 3");
		  		  	var receipt = await factory.deployContract({from: account_five, gas:4000000});
